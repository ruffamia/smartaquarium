<?php
  ...
  if (!empty($_POST)) {
    ...
    $led_01 = $_POST['led_01'];
    $led_02 = $_POST['led_02'];
    $feed = $_POST['feedManual'];
    $phAlert = $_POST['phAlert'];
    $ammoniaAlert = $_POST['ammoniaAlert'];
    //........................................
    ...
    $sql = "UPDATE data_sensor SET temperature = ?, ammonia = ?, phlvl =?, status_read_sensor_dht11 = ?,stat = ?, phalert = ?,ammoniaalert = ?,time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($temperature,$ammonia,$phlvl,$status_read_sensor_dht11,$feed,$phAlert,$ammoniaAlert,$tm,$dt,$id));
    Database::disconnect();
    //........................................ 
    ...
    while ($found_empty == false) {
      ...
      $sql = 'SELECT * FROM data_sensor WHERE id="' . $id_key . '"';
      $q = $pdo->prepare($sql);
      $q->execute();
    ...
    // This table is also used to store and record the state of the LEDs, the state of the LEDs is controlled from the "home.php" page. 
    // This table is operated with the "INSERT" command, so this table will contain many rows.
		$sql = "INSERT INTO table_sensor (id,board,temperature,ammonia,phlvl,status_read_sensor_dht11,LED_01,LED_02,stat,phalert,ammoniaalert,time,date) values(?, ?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_key,$board,$temperature,$ammonia,$phlvl,$status_read_sensor_dht11,$led_01,$led_02,$feed,$phAlert,$ammoniaAlert,$tm,$dt));
    //::::::::
